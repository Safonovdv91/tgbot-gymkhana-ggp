...
def init_logger(name, sh_level: int = 30, fh_level: int = 30):
    ...
    fh = logging.handlers.RotatingFileHandler(filename=f"logger/err_log.log")   # Использование прокаченного логгр-хэндлера
    fh.setFormatter(logging.Formatter(FORMAT))
    fh.setLevel(fh_level)

    full_log = logging.handlers.RotatingFileHandler(filename=f"logger/app_log.log")
