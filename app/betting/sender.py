...
class BettingMessageSender:
    ...
    async def sort_by_delta_time(
        ...
        sorted_users = sorted(bet_users, key=time_differense)
        return sorted_users
    ...
    async def get_sorted_bets() -> str:
        ...
        text = "Сортированные ставки:\n"
        for i in range(len(bet_users)):
            text += (
                f"\t{i + 1}) {bet_users[i].tg_user.username}"
                f" - {BotFunction.msec_to_mmssms(bet_users[i].bet_time1)} [{bet_users[i].delta_bet_time1}мс]\n"
            )
        return text
